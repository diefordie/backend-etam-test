...
export const submitFinal = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            message: 'Jawaban final berhasil disubmit',
            data: {
                resultId: result.id,
        ...
        console.error('Error in submitFinalAnswersController:', error);
        ...
        if (error.message === 'Tidak ada result yang ditemukan.' || 
            error.message === 'Tidak ada jawaban draft yang ditemukan untuk result ini.') {
        ...
        return res.status(500).json({
            ...
            message: 'Terjadi kesalahan saat memproses jawaban final',
        });
    }
};
